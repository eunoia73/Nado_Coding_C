...
>    `int* ptr = arr;`
포인터변수에 배열 넣기 -> 배열의 첫 번째 요소의 주소를 가리키게 됨
`*(ptr + i)` : `ptr[i]`와 같은 표현. 배열 첫 번째 요소의 주소로부터 i번째에 해당하는 요소의 값을 가져온다. `arr[0] = *(arr + 0)`
`arr = &arr[0]` : arr자체의 값과 arr배열 첫 요소의 주소는 같다. 
`*&arr[0]` : *와 &를 같이 사용하면 둘은 상쇄돼서 아무것도 안 붙인 것과 같다.
