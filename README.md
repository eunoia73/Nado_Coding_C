...
- 파일 포인터 : 커서가 깜빡이는 위치를 담은 것(무슨 파일 열었는지, 어디까지 열었는지, 파일 끝에 도달했는지 등의 정보를 담는다)
- 파일모드 : 파일을 여는 방식<br>
> 
`fputs(문자열, 파일포인터);` : 파일에 데이터를 문자열 단위로 저장
`fclose(파일포인터);` : 데이터 손실을 방지하기 위해 fopen()후 fclose()사용 필요
...
> `fprintf(파일포인터, "서식지정자", 값);` : 파일에 데이터 저장하기
